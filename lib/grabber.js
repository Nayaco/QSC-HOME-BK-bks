...
function xiannvsql(config, Reg){
    ...
    this.linscipherdata  = (table, data, ciphercol, passkey, method) =>{
        if(!util.isString(table) || !util.isObject(data) || !util.isString(ciphercol) || !util.isString(passkey)){
            return new Promise((resolve,reject)=>{
                const err = {
                    err: 'Input type error or lacked input'
                }
                reject(err)
            })
        }
    ...
    this.lshowtable = (table, distinct, setting) =>{
        if(!util.isString(table) || !(util.isString(setting) || util.isUndefined(setting))){
            return new Promise((resolve, reject)=>{
                const err = {
                    err: 'Inout type error'
                }
                reject(err)
            })
        }
        const order = setting?`ORDER BY ${setting}`:''
        return this.mp.query(`SELECT ${distinct===true?'DISTINCT ':''}* FROM ${table} ${order}`)
    }
    ...
    this.lgetdata = (table ,target , _sql)=>{
        if(!util.isString(table) || !util.isString(target)){
            return new Promise((resolve,reject)=>{
                const err = {
                    err: 'Input type error'
                }
        ...
        return this.mp.query(`SELECT ${target} FROM ${table} ${_sql || ''}`)
    }
